...
export default function RegisterPage() {
  ...
  return (
    <div className="mx-auto max-w-md space-y-6">
      <Helmet>
        <title>{ru.auth.metaRegister}</title>
      </Helmet>
      ...
      <div className="rounded-3xl border border-border/60 bg-surface-elevated/40 p-6 shadow-glass backdrop-blur-xl">
        ...
        <p className="mt-2 text-sm text-slate-400">{ru.auth.registerSubtitle}</p>
        ...
        >
          <div>
            <label className="mb-1 block text-xs font-medium text-slate-400" htmlFor="reg-email">
              {ru.auth.email}
            </label>
          ...
          <div>
            <label className="mb-1 block text-xs font-medium text-slate-400" htmlFor="reg-password">
              {ru.auth.password}
            </label>
            <Input
              id="reg-password"
        ...
        <p className="mt-4 text-center text-sm text-slate-400">
          {ru.auth.hasAccount}{' '}
          <Link className="font-semibold text-accent hover:text-accent/90" to="/login">
            {ru.nav.login}
          </Link>
        </p>
      </div>
